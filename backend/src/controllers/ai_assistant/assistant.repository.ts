...
import { ParsedIntent, ParsedIntentSearch } from "../../models/assistant";
...
export const AssistantProductQuery = async (
    ...
    const { keyword, gender, style, occasion, season, vibe_keywords } = intent;
    ...
    if (style) {
        scoringParts.push(`CASE WHEN p.ai_tags->>'style' = $${i} THEN 1.5 ELSE 0 END`);
        values.push(style);
        i++;
    }

    if (occasion) {
        scoringParts.push(`CASE WHEN p.ai_tags->>'occasion' = $${i} THEN 1.5 ELSE 0 END`);
        values.push(occasion);
        i++;
    }

    if (season) {
        scoringParts.push(`CASE WHEN p.ai_tags->>'season' = $${i} THEN 1.0 ELSE 0 END`);
        values.push(season);
        i++;
    }

    if (vibe_keywords && vibe_keywords.length > 0) {
        // Boost product score by 0.5 for each matching element inside the JSONB array string
        scoringParts.push(`(
            SELECT COALESCE(COUNT(*), 0) * 0.5 
            FROM unnest($${i}::text[]) AS vk 
            WHERE p.ai_tags->'vibe_keywords' ? vk
        )`);
